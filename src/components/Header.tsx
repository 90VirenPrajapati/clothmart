...
const Header = () => {
    ...
    const {productData, orderData, wishlist} = useSelector(
    ...
    return (
        <div className="bg-bodyColor h-20 top-0 sticky z-50">
            <Container className="h-full flex items-center md:gap-x-5 justify-between md:justify-start">
                ...
                {/* Search Field */}
                <div
                    className="w-full bg-white hidden md:flex items-center gap-x-1 border-[1px] border-lightText/50 rounded-full px-4 py-1.5 focus-within:border-orange-600 group">
                ...
                {session && (
                    <div onClick={() => wishlist()} className="headerDiv px-2 gap-x-1 cursor-pointer">
                        <MdFavoriteBorder className="text-2xl"/>
                        <p className="text-sm font-semibold">Wishlist</p>
                        {/* Display wishlist items here */}
                        {wishlist.map((item: Products) => (
                            <div key={item._id}>
                                <p>{item.title}</p>
                            </div>
                        ))}
